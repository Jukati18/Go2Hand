...
import {
    ShoppingBagIcon, TagIcon, ChevronRightIcon, ClockIcon,
        ...
        <div className="min-h-screen bg-[#F4F2EE]">
            ...
            <div className="max-w-[1160px] mx-auto px-4 sm:px-6 py-8 sm:py-10">
                ...
                <div className="mb-6 sm:mb-7">
                    ...
                    <p className="text-sm text-gray-400 mt-1">Track your purchases and sales with escrow status</p>
                </div>

                {/* ── Buyer / Seller tab switch ── */}
                <div className="flex items-center gap-1 bg-white border border-gray-100 rounded-xl p-1 w-fit mb-5 sm:mb-6 shadow-sm">
                    {[
                        ...
                        <button key={id} onClick={() => handleTabChange(id)}
                            className={`flex items-center gap-2 px-4 sm:px-5 py-2 rounded-lg text-sm font-semibold transition-all duration-200
                                ${tab === id ? 'bg-teal-800 text-white shadow-sm' : 'text-gray-500 hover:text-gray-800 hover:bg-gray-50'}`}>
                            <Icon className="w-4 h-4" />{label}
                        </button>
                    ))}
                </div>

                {/* ── Status filter pills — wraps on mobile ── */}
                ...
                {loading && (
                    <div className="flex flex-col gap-3">
                        {[1, 2, 3].map(i => (
                            <div key={i} className="bg-white rounded-2xl border border-gray-100 h-24 sm:h-28 animate-pulse"
                ...
                {!loading && orders.length > 0 && (
                    <div className="flex flex-col gap-3">
                        {orders.map((order, i) => (
                            <OrderRow key={order.id} order={order} role={tab}
                ...
                {!loading && orders.length === 0 && (
                    <div className="flex flex-col items-center justify-center py-20 sm:py-24 text-center">
                        ...
                        <Link href={tab === 'buyer' ? '/devices' : '/sell'}
...
function OrderRow({ order, role, style }: {
    order: Order; role: 'buyer' | 'seller'; style?: React.CSSProperties
}) {
    ...
    return (
        <Link href={`/orders/${order.id}`}
            className="group bg-white rounded-2xl border border-gray-100 shadow-sm
                ...
                p-4 sm:p-5 flex items-center gap-3 sm:gap-5 animate-[fadeUp_.35s_ease_both]"
            style={style}>

            {/* Device thumbnail */}
            <div className="w-12 sm:w-16 h-12 sm:h-16 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-center shrink-0 overflow-hidden">
                {product?.images?.[0] ? (
            ...
            <div className="flex items-center gap-2 sm:gap-4 shrink-0">
                <div className="text-right">
                    <p className="text-sm sm:text-base font-bold text-gray-900">
                        {formatOrderAmount(
                            role === 'buyer' ? order.total : order.amount - order.platformFee
                        )}
                    </p>
                    <p className="text-[10px] text-gray-400 mt-0.5">{role === 'buyer' ? 'paid' : 'payout'}</p>
                </div>
                <ChevronRightIcon className="w-4 h-4 text-gray-300 group-hover:text-teal-500 transition-colors" />
            </div>
        </Link>
    )
}